...
export class LocatorStore {
  ...
  private onLocationChanged(current: CurrentLocation): void {
    window.dispatchEvent(
      new CustomEvent(LocatorStore.LOCATION_CHANGED_NAME, {
        detail: current,
      })
    );
  }
  ...
  private setRoute(routes: RouteExt[], parentPath?: string): RouteObject[] {
    return routes.map((r: RouteExt) => {
      ...
      if (parentPath?.endsWith("/:id?"))
        throw new Error(
          "You cannot use 'useParam' with 'children' in a Route."
        );
      ...
      route.loader = async ({ request, params }) => {
        ...
        const fullPaths = decodeURIComponent(url.pathname)
      ...
      if (children) {
        ...
        (route as RouteObject).children = this.setRoute(
          children,
          this.keyPath.get(key)
        );
      }
