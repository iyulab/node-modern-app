...
import styles from "@iyulab/modern-app/styles/layouts/PageBase.module.scss?inline";
...
function PageBase(props: PageBaseProps) {
  ...
  const adjustRows = () => {
    ...
    const bottomPanelHeight =
      isFixedBottom && bottomPanel.current
        ? bottomPanel.current.offsetHeight
        : 0;
    const bottomMenuHeight = bottomMenu.current
      ? bottomMenu.current.offsetHeight
      : 0;
  ...
  const adjustColumns = () => {
    ...
    const rightPanelWidth =
      isFixedRight && rightPanel.current ? rightPanel.current.offsetWidth : 0;
    const rightMenuWidth = rightMenu.current
      ? rightMenu.current.offsetWidth
      : 0;
  ...
  const resizeRightPanel = (event: any) => {
    ...
    const stop = () => {
      ...
      rightPanel.current!.style.width = `${
        rightPanel.current!.offsetWidth - moveX
      }px`;
      adjustColumns();
  ...
  const resizeBottomPanel = (event: any) => {
    ...
    const stop = () => {
      ...
      target.classList.remove(styles.active);
      bottomPanel.current!.style.height = `${
        bottomPanel.current!.offsetHeight - moveY
      }px`;
      adjustRows();
  ...
  return (
    <div className={styles.container} ref={container}>
      ...
      <div className={styles.main} ref={main}>
        ...
        >
          <svg className={styles.icon} viewBox="0 0 24 24">
            <path d={VectorIcons.angleUp2}></path>
          </svg>
        </div>
      </div>
      ...
      {/* 페이지 오른쪽 메뉴 */}
      {rightKeys.length > 0 && (
        <div className={`${styles.menu} ${styles.right}`} ref={rightMenu}>
          {rightKeys.map((key) => (
            <div
              key={key}
              className={`${styles.tab} ${
                rightKey === key ? styles.active : ""
              }`}
      ...
      {bottomKey !== null && (
        <div
          className={`${styles.panel} ${styles.bottom} ${
            isFixedBottom ? styles.fixed : ""
          }`}
          ref={bottomPanel}
        >
          <div className={styles.resize} onMouseDown={resizeBottomPanel}></div>
          <div className={styles.panelHead}>
            <div className={styles.title}>{bottomKey}</div>
            <div className={styles.flex}></div>
            <svg
              className={`${styles.button} ${styles.pin}`}
              viewBox="0 0 24 24"
      ...
      {/* 페이지 하단 메뉴 */}
      {bottomKeys.length > 0 && (
        <div className={`${styles.menu} ${styles.bottom}`} ref={bottomMenu}>
          {bottomKeys.map((key) => (
            <div
              key={key}
              className={`${styles.tab} ${
                bottomKey === key ? styles.active : ""
              }`}
